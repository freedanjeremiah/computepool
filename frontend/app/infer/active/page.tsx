...
const STREAM_TEXT = `Attention is how a transformer decides what to focus on.
...
export default function InferStep4() {
  ...
  React.useEffect(() => {
    const id = setInterval(() => setChars((c) => Math.min(c + 8, STREAM_TEXT.length)), 60);
  ...
  React.useEffect(() => {
    const id = setTimeout(() => setBreached(true), 7000);
    return () => clearTimeout(id);
  }, []);
  React.useEffect(() => {
    if (chars >= STREAM_TEXT.length) {
      const id = setTimeout(() => {
        setState({ ...state, output: STREAM_TEXT, breached });
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20 }}>
      <Card padding={32} style={{ minHeight: 680 }}>
        ...
        <div style={{ marginTop: 20, fontFamily: FONT_MONO, fontSize: 15, color: T.text1, lineHeight: 1.65, whiteSpace: "pre-wrap" }}>
          ...
          {chars < STREAM_TEXT.length && (
            <span style={{
              display: "inline-block", width: 8, height: 18,
              background: T.primary, marginLeft: 2, verticalAlign: "-3px",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        <Card padding={0} style={{ overflow: "hidden" }}>
          ...
          <svg width="100%" viewBox="0 0 380 220" style={{ display: "block" }}>
            ...
            {[0, 1, 2].map((i) => {
              const phase = ((tickN * 0.012 + i * 0.33) % 1);
              const x = 290 + (90 - 290) * phase;
              const y = 110 + (65 - 110) * phase;
              return <circle key={"pa" + i} cx={x} cy={y} r="2.5" fill={T.primary} opacity={1 - phase * 0.5}/>;
            })}
            {!breached && [0, 1, 2].map((i) => {
              const phase = ((tickN * 0.012 + i * 0.33 + 0.5) % 1);
              const x = 290 + (90 - 290) * phase;
              const y = 110 + (155 - 110) * phase;
              return <circle key={"pb" + i} cx={x} cy={y} r="2.5" fill={T.primary} opacity={1 - phase * 0.5}/>;
            })}
          </svg>
        </Card>
        ...
        <Card padding={24}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <Badge kind="primary" label="live"/>
          </div>
          <NodeRow id="node-a" hex="0xaaa…1234" rate={`+${aBal.toFixed(4)}`} frozen={false}/>
          <div style={{ height: 8 }}/>
          <NodeRow id="node-b" hex="0xbbb…5678" rate={breached ? "—" : `+${bBal.toFixed(4)}`} frozen={breached}/>
          {breached && (
          ...
          <div style={{ marginTop: 18, paddingTop: 14, borderTop: `1px solid ${T.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              ...
              <span style={{ fontFamily: FONT_MONO, fontSize: 20, fontWeight: 500, color: T.text1, whiteSpace: "nowrap" }}>
                ...
                <span style={{ color: T.text3, fontSize: 12 }}>/ {totalBudget.toFixed(2)}</span>
              </span>
            </div>
            <div style={{ marginTop: 10, height: 5, background: T.border, borderRadius: 3, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(totalSpent / totalBudget) * 100}%`, background: T.primary, transition: "width 200ms linear" }}/>
            </div>
          </div>
